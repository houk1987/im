...
public class CustomTree extends PubTree implements MouseListener,ComponentListener {
    ...
    public void loadCustomTreeData() {
        try {
            List<GroupChatInfo> groupChatInfoList = GroupChatManager.getAllGroupChat(); //获取服务器上所有的会议室
            if (groupChatInfoList != null && groupChatInfoList.size() > 0) {
                for (GroupChatInfo groupChatInfo : groupChatInfoList) {
                        System.out.println(groupChatInfo.getOwner());
                        if(groupChatInfo.getOwner()!=null && groupChatInfo.getOwner().equals(ConnectionManager.getConnection().getFullUserName())){
            ...
            if (component instanceof DefaultMutableTreeNode) {
                ...
                if (e.getButton() == 3 && this.isPathSelected(path)) {
                    ...
                    if (e.getClickCount() == 2) {
                        ...
                        if(object instanceof GroupChatInfo){
                            GroupChatInfo groupChatInfo = (GroupChatInfo)object;
                            ChatWindowManager.openGroupChatWindow(groupChatInfo);
                        }else if(object instanceof UserInfo){
                            UserInfo userInfo = (UserInfo)object;
                            if(!userInfo.getId().equals(DtClientWindow.getInstance().getUserInfo().getId())){
                                ChatWindowManager.openSingleChatWindow(userInfo.getId());
                            }
                        }
                    }
                }
