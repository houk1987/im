    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == createCustomGroupItem){
            ...
            try {
                ...
                int rs = JOptionPane.showConfirmDialog(DtClientWindow.getInstance(), "是否删除群+"+groupChatInfo.getName()+"删除成功!");
                if(rs == 0) {
                    DtClientWindow.getInstance().getCustomTree().deleteNode(node);
                    JOptionPane.showMessageDialog(DtClientWindow.getInstance(), "删除群组成功!");
                }
